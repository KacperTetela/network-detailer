...
import networkdetailer.com.model.api.DatabaseDTO;
import networkdetailer.com.model.api.HttpClient;
...
public class DataCollector {
  ...
  public DataCollector() {
    networkDownloader =
        new NetworkDownloader(new IPGetter(), new MacGetterOnline(), new MacGetterOffline());
  ...
  public int saveToExcel() {
    /** int = 0 Error int = 1 Success int = 2 Hostname already exists */
    Path jarPath;
    try {
      jarPath =
          Paths.get(DataCollector.class.getProtectionDomain().getCodeSource().getLocation().toURI())
              .getParent();
    ...
    try {
      if (file.exists()) {
        ...
        for (int i = 1; i <= rowCount; i++) {
          ...
          if (row != null
              && row.getCell(0) != null
              && row.getCell(0).getStringCellValue().equals(networkData.hostname())) {
            // Update existing row
  ...
  public boolean saveToTxt() {
    try {
      ...
      writer.write(
          "CPU Manufacturer: " + cpuData.generation().cpuManufacturer() + System.lineSeparator());
  ...
  public boolean syncWithServer() {
    DatabaseDTO databaseDTO =
        new DatabaseDTO(
            networkData.hostname(),
            networkData.ip(),
            networkData.mac(),
            cpuData.generation().cpuManufacturer().toString(),
            cpuData.name(),
            cpuData.generation().generation(),
            cpuData.ghz(),
            memoryData.ramGB(),
            memoryData.diskSpaceGB(),
            memoryData.diskType().toString(),
            moboData.bios(),
            (isWindowsRequirements() ? "Yes" : "No"));
    return HttpClient.getInstance().sendData(databaseDTO);
  }
